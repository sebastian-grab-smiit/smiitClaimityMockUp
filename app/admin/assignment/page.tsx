...
import { ArrowLeft, Users, MapPin, Star, Clock, Search, UserPlus, Phone, Mail } from "lucide-react"
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-7xl mx-auto p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-slate-800 mb-2">Expertenzuweisung</h1>
          <p className="text-slate-600">Weisen Sie bereite Fälle an verfügbare Experten zu</p>
        </div>
                  ...
                  {readyCases.map((case_) => (
                    <div
                      ...
                      className={`p-3 border rounded-lg cursor-pointer transition-colors ${
                        selectedCase === case_.id ? "bg-teal-50 border-teal-200" : "hover:bg-slate-50"
                      }`}
                  ...
                  {filteredExperts.map((expert) => (
                    <div
                      ...
                      className={`p-4 border rounded-lg cursor-pointer transition-colors ${
                        selectedExpert === expert.id ? "bg-blue-50 border-blue-200" : "hover:bg-slate-50"
                      }`}
        ...
        {selectedCase && selectedExpert && (
          <Card className="mt-6 border-teal-200 bg-teal-50">
            <CardHeader>
              <CardTitle className="text-teal-800">Zuweisung bestätigen</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <h4 className="font-semibold text-slate-800 mb-2">Fall</h4>
                  <p className="text-slate-700">
                    {selectedCaseData?.id} - {selectedCaseData?.type}
                  </p>
                  <p className="text-sm text-slate-600">{selectedCaseData?.location}</p>
                </div>
                <div>
                  <h4 className="font-semibold text-slate-800 mb-2">Experte</h4>
                  <p className="text-slate-700">{selectedExpertData?.name}</p>
                  <p className="text-sm text-slate-600">{selectedExpertData?.location}</p>
                </div>
              </div>
            </CardContent>
          </Card>
