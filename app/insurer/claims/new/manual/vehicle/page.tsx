...
import {
  ...
  Settings, Download, BarChart3, Plus, MessageSquare, Car
} from "lucide-react"
...
export default function NewClaimPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    damageDescription: "",
    estimatedAmount: "",
    inspectionType: "", // 'werkstatt' | 'privat' | 'kvp' | 'rechn'
    workshop: { name: "", address: "", postalCode: "", city: "", phone: "" } as Party,
    inspectionDate: "",
  ...
  // Abgeleitete Kontakt-/Adressdaten für "Privatbesichtigung"
  const privateParty: Party = isHaftpflicht ? formData.counterparty : formData.insured
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="mb-10 text-center">
            ...
            <h1 className="text-2xl font-bold text-slate-800 mb-4">Fahrzeuggutachter</h1>
            ...
            <CardContent>
              ...
              {currentStep === 1 && (
                <div className="space-y-8">
                  <div className="grid md:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="claimType">Schadensart (Freitext)</Label>
                      <Input
                        id="claimType"
                        placeholder="z. B. Parkschaden, Glasbruch, ..."
              ...
              {currentStep === 3 && (
                <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="privat">Privatbesichtigung</SelectItem>
                  ...
                  {/* Werkstatt-Felder */}
                  {formData.inspectionType === "werkstatt" && (
                    <div className="space-y-4 rounded-lg border bg-slate-50 p-4">
                      <h4 className="font-medium text-slate-800">Werkstattdaten</h4>
                      <div className="grid md:grid-cols-2 gap-4">
                        <div className="space-y-2 md:col-span-2">
                          ...
                          <Input
                            ...
                            onChange={(e) => setFormData({ ...formData, workshop: { ...formData.workshop, name: e.target.value } })}
                          />
                        </div>
                        <div className="space-y-2 md:col-span-2">
                          ...
                          <Input
                            id="ws-address"
                            placeholder="Strasse und Nr."
                            value={formData.workshop.address}
                            onChange={(e) => setFormData({ ...formData, workshop: { ...formData.workshop, address: e.target.value } })}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="ws-postal">PLZ *</Label>
                          <Input
                            id="ws-postal"
                            placeholder="8000"
                            value={formData.workshop.postalCode}
                            onChange={(e) => setFormData({ ...formData, workshop: { ...formData.workshop, postalCode: e.target.value } })}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="ws-city">Ort *</Label>
                          <Input
                            id="ws-city"
                            placeholder="Zürich"
                            value={formData.workshop.city}
                            onChange={(e) => setFormData({ ...formData, workshop: { ...formData.workshop, city: e.target.value } })}
                          />
                        </div>
                        <div className="space-y-2 md:col-span-2">
                          <Label htmlFor="ws-phone">Telefonnummer *</Label>
                          <Input
                            id="ws-phone"
                            placeholder="+41 ..."
                            value={formData.workshop.phone}
                            onChange={(e) => setFormData({ ...formData, workshop: { ...formData.workshop, phone: e.target.value } })}
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  {/* Privatbesichtigung - nur Anzeige (übernommene Daten) */}
                  {formData.inspectionType === "privat" && (
                    <div className="space-y-2 rounded-lg border bg-slate-50 p-4">
                      <h4 className="font-medium text-slate-800">Privatbesichtigung – Adresse wird automatisch übernommen</h4>
                      <div className="grid md:grid-cols-2 gap-4">
                        <div className="space-y-2 md:col-span-2">
                          <Label>Person</Label>
                          <Input value={privateParty.name} disabled />
                        </div>
                        <div className="space-y-2 md:col-span-2">
                          <Label>Adresse</Label>
                          <Input value={privateParty.address} disabled />
                        </div>
                        <div className="space-y-2">
                          <Label>PLZ</Label>
                          <Input value={privateParty.postalCode} disabled />
                        </div>
                        <div className="space-y-2">
                          <Label>Ort</Label>
                          <Input value={privateParty.city} disabled />
                        </div>
                        <div className="space-y-2 md:col-span-2">
                          <Label>Telefon</Label>
                          <Input value={privateParty.phone} disabled />
                        </div>
                      </div>
                    </div>
                  )}
              ...
              {/* Schritt 4: Dokumente (genau so lassen) */}
              {currentStep === 4 && (
                <div className="space-y-4">
              ...
              {/* Schritt 5: Review (wie erster Entwurf) */}
              {currentStep === 5 && (
                <div className="space-y-8">
                  <Alert>
                        ...
                        <ReviewRow
                          ...
                          value={
                            formData.claimInsurance === "hp"
                              ? "Haftpflicht"
                              : formData.claimInsurance === "tk"
                              ? "Teilkasko"
                              : formData.claimInsurance === "vk"
                              ? "Vollkasko"
                              : undefined
                          }
                        />
                        <ReviewRow label="Schadensart" value={formData.claimType} />
                      </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="rounded-lg border bg-white p-4 space-y-3">
                        ...
                        <ReviewRow
                          ...
                          value={
                            formData.inspectionType === "werkstatt" ? "Werkstatt" :
                            formData.inspectionType === "privat" ? "Privatbesichtigung" :
                            formData.inspectionType === "kvp" ? "Kostenvoranschlag prüfen" :
                            formData.inspectionType === "rechn" ? "Rechnung prüfen" : undefined
                          }
                        />

                        {/* Je nach Art Zusatzinfos */}
                        {formData.inspectionType === "werkstatt" && (
                          <>
                            <ReviewRow label="Werkstatt-Name" value={formData.workshop.name} />
                            <ReviewRow label="Werkstatt-Adresse" value={formData.workshop.address} />
                            <ReviewRow label="Werkstatt-PLZ" value={formData.workshop.postalCode} />
                            <ReviewRow label="Werkstatt-Ort" value={formData.workshop.city} />
                            <ReviewRow label="Werkstatt-Telefon" value={formData.workshop.phone} />
                          </>
                        )}

                        {formData.inspectionType === "privat" && (
                          <>
                            <div className="text-slate-600 mt-2">Privatbesichtigung – verwendete Kontaktdaten</div>
                            <ReviewRow label="Name" value={privateParty.name} />
                            <ReviewRow label="Adresse" value={privateParty.address} />
                            <ReviewRow label="PLZ" value={privateParty.postalCode} />
                            <ReviewRow label="Ort" value={privateParty.city} />
                            <ReviewRow label="Telefon" value={privateParty.phone} />
                          </>
                        )}
